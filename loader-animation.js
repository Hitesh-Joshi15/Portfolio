...
class LoaderAnimation {
    constructor() {
        ...
        this.assetHardTimeoutMs = 3500; // slow disk/net must not stall the show
        this.logoLoadFailed = false;
        this.logoGateBypassed = false; // skip-click may jump the logo gate
        this.liteMode = false; // flips on when frames run long (low-spec machines)
        this.frameCostAccum = 0;
        this.frameCostSamples = 0;
        ...
        this.logoPostFormHoldMs = 500; // hold the finished logo this long before morphing to the name
        this.logoPinStartedAt = 0;
        this.logoPinMaxMs = 6000; // safety valve: never pin forever on a struggling machine
    ...
    handleSkipClick(e) {
        ...
        if (this.elapsed < logoPauseEnd) {
            ...
            this.logoGateBypassed = true; // deliberate jump past the logo gate
    ...
    resizeCanvas() {
        ...
        this.dpr = this.liteMode ? 1 : Math.min(window.devicePixelRatio || 1, 1.75);
    ...
    update(dt) {
        ...
        // GATE: the timeline may not cross the logo-stage boundary until the
        // logo is fully formed AND has been held for logoPostFormHoldMs.
        // Absolute pin (not dt-based) so slow machines can't leak past it.
        const logoBoundary = this.phaseDurations[0] + this.phaseDurations[1] - 1;
        const logoHoldDone = this.logoIsStable && this.logoFullyFormedAt > 0 &&
            (this.now - this.logoFullyFormedAt >= this.logoPostFormHoldMs);
        if (this.currentStage <= 1 && !this.logoGateBypassed && !logoHoldDone && this.elapsed > logoBoundary) {
            if (!this.logoPinStartedAt) this.logoPinStartedAt = this.now;
            if (this.now - this.logoPinStartedAt < this.logoPinMaxMs) {
                this.sequenceStartTime = this.now - logoBoundary;
                this.elapsed = logoBoundary;
            }
        }
        ...
        if (stage === 1) {
            ...
            if (logoHoldSatisfied) {
                this.logoPinStartedAt = 0; // gate released — timeline flows into the morph
            }
        }
    ...
    updateLogoFormationState(logoTarget) {
        ...
        // The longer the gate has pinned the timeline, the more lenient the
        // "formed" test gets — weak machines still get their lock + hold.
        const pinAge = this.logoPinStartedAt ? this.now - this.logoPinStartedAt : 0;
        const relaxed = pinAge > 2500;
        const coverageTarget = relaxed ? 0.82 : this.logoCoverageTarget;
        const avgTighten = relaxed ? 1.7 : this.logoAvgTighten;
        const stableTarget = relaxed ? 7 : this.logoStableFrameTarget;

        const avgDistSq = sumDistSq / samples;
        const avgThresholdSq = this.logoFormationThresholdSq * avgTighten;
        const coverage = inToleranceCount / samples;
        const isFullyFormed = avgDistSq <= avgThresholdSq && coverage >= coverageTarget;

        if (isFullyFormed) {
            this.logoStableFrames++;
            if (this.logoStableFrames >= stableTarget) {
    ...
    drawParticles() {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            ...
            const glowEvery = (i % (this.liteMode ? 21 : 7)) === 0;
    ...
    animate() {
        ...
        const rawDt = this.now - this.lastFrameTime;
        const dt = Math.min(34, Math.max(8, rawDt));
        this.lastFrameTime = this.now;

        this.update(dt);
        this.draw();
        this._notePerf(rawDt);

        this.rafId = requestAnimationFrame(() => this.animate());
    }

    // Rolling frame-cost check: consistently slow frames flip on lite mode
    // (1x resolution + thinner decoration) instead of letting the show crawl.
    _notePerf(rawDt) {
        if (this.liteMode || this.transitionStarted || rawDt > 250) return; // ignore tab-switch spikes
        this.frameCostAccum += rawDt;
        this.frameCostSamples++;
        if (this.frameCostSamples >= 40) {
            if (this.frameCostAccum / this.frameCostSamples > 27) {
                this._enableLiteMode();
            }
            this.frameCostAccum = 0;
            this.frameCostSamples = 0;
        }
    ...
    _enableLiteMode() {
        ...
        // Thin the decorative layers only — the name/logo particles stay intact.
        this.ambient = this.ambient.filter((_, i) => i % 2 === 0);
        this.webNodes = this.webNodes.filter((_, i) => i % 2 === 0);
        this.webSquares = this.webSquares.filter((_, i) => i % 2 === 0);
    }
